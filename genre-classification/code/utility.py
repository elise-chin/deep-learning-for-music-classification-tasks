...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, filenames, labels, batch_size, input_shape, n_channels=None):
...
def data_split(X, y, train_size, random_state=42):
    ...
    for train_index, tmp_index in splits:
        X_train, X_tmp = X[train_index], X[tmp_index]
        y_train, y_tmp = y[train_index], y[tmp_index]
    
    splitter = StratifiedShuffleSplit(n_splits=1, train_size=0.5, random_state=random_state)
    splits = splitter.split(X_tmp, y_tmp)

    for val_index, test_index in splits:
        X_val, X_test = X_tmp[val_index], X_tmp[test_index]
        y_val, y_test = y_tmp[val_index], y_tmp[test_index]

    return X_train, X_val, X_test, y_train, y_val, y_test


def plot_history(history_dict, num_epochs, save_path=None):
    ...
    plt.plot(history_dict['val_accuracy'], marker='d', label='Validation accuracy')
